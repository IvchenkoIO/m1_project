...
function displayConversation(conversationId) { const conversation = conversations[conversationId]; if (!conversation) { console.error(`Conversation "${conversationId}" not found.`); return; } messagesDiv.innerHTML = ""; for (let message of conversation) { if (message.role !== "system") { const messageElem = document.createElement("div"); messageElem.classList.add(message.role); messageElem.innerHTML = `${message.role}: ${message.content}`; messagesDiv.appendChild(messageElem); } } }
